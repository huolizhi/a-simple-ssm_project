							...
							<tbody>
								...
								<c:forEach items="${pList}" var="p">
									<tr>
										...
										<td>${p.productName}</td>
										<td>${p.cityName}</td>
										<td><%--<fmt:formatDate value="${p.departureTime}" pattern="yyyy-MM-dd hh:mm"></fmt:formatDate>--%>
											<fmt:formatDate value="${p.departureTime}" pattern="yyyy-MM-dd HH:dd"></fmt:formatDate>
										</td>
										...
										<td class="text-center">
											<button type="button" class="btn bg-olive btn-xs"
													onclick='location.href="${pageContext.request.contextPath}/product/initUpdate.do?id=${p.id}"'>修改</button>
											<button type="button" class="btn bg-olive btn-xs"
													onclick="f1(${p.id})">删除</button>
										</td>
									</tr>
								</c:forEach>
